...
//  Snippets
//
//  Created by 温蟾圆 on 2021/2/10.
//

import UIKit

class TimeScrollViewController: UIViewController {
    
    lazy var animatedView = NumberScrollAnimatedView()
    ...
    override func viewDidLoad() {
        ...
        animatedView = NumberScrollAnimatedView(frame: CGRect(x: 0, y: 100, width: 300, height: 100))
        view.addSubview(animatedView)
        animatedView.backgroundColor = .systemBlue
        animatedView.font = .systemFont(ofSize: 32)
        animatedView.animationDuration = 3
        animatedView.animationDurationOffsetRule = { _, _ in
            return 0
        }
        animatedView.scrollingDirectionRule = { _, _ in
            return .down
        }
        animatedView.inverseSequenceRule = { _, _ in
            return true
        }
        let gesture = UITapGestureRecognizer(target: self, action: #selector(animated))
        view.addGestureRecognizer(gesture)
    }
    
    @objc func animated() {
        animatedView.text = beginNum.description
...
public class NumberScrollAnimatedView: UIView {
    ...
    fileprivate func createAnimations() {
        for (index, column) in scrollableColumns.enumerated() {
            column.createAnimation(timeOffset: animationTimeOffsetRule(text, index),
